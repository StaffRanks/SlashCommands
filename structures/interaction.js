...
module.exports = class extends events {
    constructor(client, data) {
        ...
        this.members = Object.keys(this.resolved.members || {}).map(key => {
            let data = Object.assign(this.resolved.members[key], {
                id: key,
                user: this.resolved.users[key]
            })
        ...
        this.optionsData.forEach(d => {
            ...
            if (d.type === 6) {
                this.options[d.name] = this.members.find(a => a.id === d.value)
            }
            if (d.type === 7) {
                this.options[d.name] = this.channels.find(a => a.id === d.value)
            }
            if (d.type === 8) {
                this.options[d.name] = this.roles.find(a => a.id === d.value)
            }
            if (d.type === 9) {
                this.options[d.name] = d.value
            }
    ...
    _check() {
        ...
        this.channel = this._client.getChannel(this.channelID) || {
            id: this.channelID
        }
    ...
    reply(content, file, type = 4) {
        ...
        return this._client.requestHandler.request("POST", url, true, this.sent ? content : {
            type: type,
            data: content
        }, file).then(res => {
            this.sent = true
