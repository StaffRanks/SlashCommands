...
if(this.options.find(o => o.name === name)) { this.options = this.options.filter(o => o.name !== name) }
...
axios.get(url , { headers: { authorization: this._client._token } }).then(res =>{
if(!res) return;
re(res.data)
}).catch(err =>{
rej(err.response)
...
return new Promise((re , rej) =>{
axios.delete(guildID ? config.endpoints.command(this.id) + "/guilds" + guildID + "/commands/" + command.id : config.endpoints.command(this.id) + "/commands/" + command.id , { headers: { authorization: this._client._token }})
.then((res) =>{
if(!res) return;
re(command)
}).catch(err =>{
